...
var strWaitMessage = "Salvando...";
var showAllPopups = true;
		...
		if($(this).attr("target") == "" || $(this).attr("target") == undefined) {
			$(this).validate({
				submitHandler: function(form) {
					var keepDoing = true;
					try{keepDoing = beforeSave();}catch(x){}
...
/**
 * Padroniza os estilos dos bot�es
 */
	...
	$.ajax({
		...
		"success": function(data) {
			if(data != null && data.messageError) {
				...
				if (showAllPopups) {
					...
					$("#message").val(data.messageError);
					fnCallback(data);
				}
			}
			else if(data != null && data.messageSuccess) {
				if (showAllPopups) {
					showDialog(TYPE_CONFIRM, "Sorteio", data.messageSuccess, function(){
						fnCallback(data);
					});
				}
				else {
					$("#message").val(data.messageSuccess);
					fnCallback(data);
				}
			}
			else {
		...
		"error": function(jqXHR, errorThrown) {
			...
			if(errorThrown == "parsererror"){
				...
				if (showAllPopups)
					showDialog(TYPE_ERROR, "Errow", errorThrown);
				else {
					fnCallback(data);
				}
			}
...
var centerScreenWidth = 0;
var centerScreenHeight = 0;
function centerScreen(height, width){
	if (height > 0 && width > 0) {
		centerScreenWidth = width;
		centerScreenHeight = height;
	}
	
	var newWidth = centerScreenWidth;
	var newHeight = centerScreenHeight;

	if (newWidth > 0 && newHeight > 0) {
		$("#divIcon").show();

		if(window.innerWidth <= 640){
			$("#divSubContent")[0].style.marginTop = "0px";
			$("#divSubContent")[0].style.marginBottom = "0px";
			$("#divSubContent")[0].style.width = (window.innerWidth - 20) +"px";
			$("#divSubContent")[0].style.height = (window.innerHeight - 90) +"px";
			$("#divSubContent")[0].style.border = "#CDCDCD 0px solid";
			$("#divSubContent")[0].style.marginLeft = "0px";
		}
		else {
			$("#divMenu").css("margin-left", ((window.innerWidth /2) - (newWidth / 2)) +"px");
			$("#divMenu").css("width", newWidth +"px");
			
			$("#divSubContent")[0].style.marginTop = "20px";
			$("#divSubContent")[0].style.marginBottom = "20px";
			$("#divSubContent")[0].style.width = newWidth +"px";
			$("#divSubContent")[0].style.height = newHeight +"px";
			$("#divSubContent")[0].style.border = "#CDCDCD 1px solid";
			$("#divSubContent")[0].style.marginLeft = ((window.innerWidth /2) - (newWidth / 2)) +"px";
		}
	}
}

function maximizeContent(){
	$("#divContent")[0].style.height = ((window.innerHeight - 80) +"px");
}
